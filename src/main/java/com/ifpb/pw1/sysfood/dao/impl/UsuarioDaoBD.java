    ...
    public Boolean criar(Usuario novo) throws PersistenciaException {
        if (novo.getId() == 0) {
            ...
            String sql = "UPDATE usuario(nome, profissao, aniversario, sexo, fotoPerfil, descricao, rua, numero, cidade," +
                    " estado, cep, telefone, senha) set(?,?,?,?,?,?,?,?,?,?,?,?) WHERE id = ?";
            try {
                PreparedStatement st = conexao.prepareStatement(sql);
                st.setString(1, novo.getNome());
                st.setString(2, novo.getProfissao());
                st.setString(3, novo.getAniversario());
                st.setString(4, novo.getSexo());
                st.setString(5, novo.getFotoPerfil());
